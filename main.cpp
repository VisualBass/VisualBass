...
void UpdateAndDrawParticles(Color orbColor) {
    ...
    float glowValue = 90.0f;  // Example intensity value, could come from audio or other sources

    // Spawn a new particle with random movement based on glowValue
    particleSystem.SpawnParticle((Vector3){ (float)GetRandomValue(0, GetScreenWidth()), (float)GetRandomValue(0, GetScreenHeight()), 0.0f }, WHITE, glowValue);

    // Update and draw particles
    particleSystem.Update(GetFrameTime(), glow_value, orbColor);
    particleSystem.Draw();
}
...
<<<<<<< HEAD
        ...
        // If in PARTICLE_MODE_01, update particles
        if (currentMode == PARTICLE_MODE_01) {
            particleSystem.Update(GetFrameTime(), glow_value, orbColor);  // Pass orbColor to particles
            particleSystem.Draw();
        }
